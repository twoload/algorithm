...
int main() {
    ...
    while (m--) {
        ...
        if (what == 1) {
            int index, val;
            cin >> index >> val;
            // update only tin[index]  (tin[index] ~ tin[index])
            update_range(tree, lazy, 1, 1, timer, tin[index], tin[index], val);
        } else if (what == 2) {
            int index;
            cin >> index;
            // sum between tin[index] ~ tout[index]  (all children's value)
            cout << sum(tree, lazy, 1, 1, timer, tin[index], tout[index]) << '\n';
        }
    }
    return 0;
}
