...
// source --(1)--> employee --> work --(1)--> sink

// inf = N(400) * 10000 = 4'000'000

// MCMF

// yulhulganho1 + cost
// ------------
//  = two parties = network flow
...
struct MCMF {
    ...
    void add_edge(int u, int v, int cap, int cost) {
        Edge *ori = new Edge(v,cap,cost); // +cost
    ...
    bool spfa() {
        ...
        distance[source] = 0; // dist[source] = 0
        queue<int> q;
        q.push(source);
        while (!q.empty()) {
            int x = q.front();
            q.pop();
            check[x] = false; // there is no x in queue
            for (int i=0; i<graph[x].size(); i++) {
                ...
                if (e->capacity > 0 && distance[x] + e->cost < distance[y]) {
                    ...
                    if (!check[y]) {
                        check[y] = true; // there is x in queue
                        q.push(y); 
                    }
                }
            }
